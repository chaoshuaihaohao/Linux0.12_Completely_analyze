...
#define SYS_SIZE 0x3000	/* 12 * 1024 */
...
static void build_bootsect(int argc, char **argv)
{
	...
	for (i = 0; i < sizeof(buf); i++)
		buf[i] = 0;
	id = open(argv[1], O_RDONLY, 0);
	if (id < 0)
		die("Unable to open 'boot'");
	i = read(id , buf, MINIX_HEADER);
	if (i != MINIX_HEADER)
...
static void build_setup(char **argv)
{
	char buf[1024];
	int c, i;
	int id;

	id = open(argv[2], O_RDONLY, 0);
	if (id < 0)
		die("Unable to open 'setup'");
	c = read(id, buf, MINIX_HEADER);
	if (c != MINIX_HEADER)
...
static void build_system(char **argv)
{
	char buf[4096];
	int c, i;
	int id;

	id = open(argv[3],O_RDONLY,0);
	if (id < 0)
